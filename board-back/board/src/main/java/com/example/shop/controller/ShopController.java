...
//                    : shopService.getPagingProduct("sold_price", p_num);
            return shopService.getPagingProduct(sort, p_num);
        }
            //return shopService.getPagingProduct("sold_price", p_num);
        else if(category.equals("전체") && searchInput.length() != 0)  // 전체, 검색O
            return shopService.getPagingProductWithKeyword(searchInput, p_num, sort);
        else if (searchInput.length() == 0) // 카테고리, 검색X
            return shopService.getPagingProductCate(category, p_num, sort);
        else  // 카테고리, 검색O
            return shopService.getPagingProductCateWithKeyword(category, searchInput, p_num, sort);
    }
